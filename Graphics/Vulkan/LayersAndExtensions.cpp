...
#define LOGTAG "VulakLayersAndExtensions"


#define CALL_VK(func)                                                 \
  if (VK_SUCCESS != (func)) {                                         \
    __android_log_print(ANDROID_LOG_ERROR, "VALIDATION ",               \
...
uint32_t LayerAndExtensions::getLayerCount(void) {
    ...
    KSLOGI(LOGTAG,"InstLayerCount = %d", count);
    return count;
}
...
bool LayerAndExtensions::getExtensionNames(std::vector<const char*>& names, void* handle) {
    ...
    if(extensions_.find(handle) == extensions_.end()) {
        KSLOGE(LOGTAG,"No extension for the physical device %p in %s", handle, __FUNCTION__);
        return false;
    }
...
bool LayerAndExtensions::isExtensionSupported(const char* extName,
    ...
    initDevExtensions(handle);
    if (extensions_.find(handle) == extensions_.end()) {
        KSLOGE(LOGTAG,"No device extension for physical device %p in %s", handle, __FUNCTION__);
        return false;
    }
...
void LayerAndExtensions::initDevExtensions(void* device) {
    ...
    for (int i = 0; i < count; i++) {
        KSLOGI(LOGTAG,"layerName: %s for device %p", properties[i].layerName, physicalDev);
...
void LayerAndExtensions::printLayers(void) {
    for (auto& layer : layerNames_) {
        KSLOGI(LOGTAG,"Available Layers for %p: ", layer.first);
        for (auto& name : layer.second) {
            KSLOGI(LOGTAG,"%s", name);
        }
    }
}
...
void LayerAndExtensions::printExtensions(void) {
    ...
    for (auto& item : extensions_) {
        for(auto& exts: item.second) {
            KSLOGI(LOGTAG,"%s extensions for device %p, layer %s", item.first? "Device" : "Instance", item.first, exts.first.c_str());
            for (auto& e : exts.second) {
                KSLOGI(LOGTAG,"    %s", e);
            }
        }
...
void LayerAndExtensions::printExtensions(const char* name, VkPhysicalDevice device) {
    ...
    KSLOGI(LOGTAG,"Instance Extensions for layer %s:", layerNameKey.c_str());
    ...
    KSLOGI(LOGTAG,"Device extension for layer %s", layerNameKey.c_str());
    for(auto& e : exts) {
        KSLOGI(LOGTAG,"    %s", e);
    }
}
 ...
 */
static VkBool32 VKAPI_PTR vkDebugReportCallbackEX_impl(VkDebugReportFlagsEXT flags, VkDebugReportObjectTypeEXT objectType,
...
{
    if (isExtensionSupported(kDbgUtilsName,VK_NULL_HANDLE, nullptr))
    {
        //TODO PFN already imported. cache
    ...
    {
        //TODO already imported ,no prototype macro, cache
        ...
        // vkDestroyDebugReportCallbackEXT either.//TODO
